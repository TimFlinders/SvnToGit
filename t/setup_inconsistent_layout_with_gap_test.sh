...
fixture_dir=$fixtures_dir/inconsistent_layout_with_gap
...
svn commit -m "Oops, forgot to include test2.txt" --username jane
svn up

echo "Add another line from trunk, rev 7" >> trunk/test.txt
svn commit -m "Update test.txt from trunk" --username joe
svn up
...
echo "Add another line from foo, rev 9" >> branches/foo/test.txt
