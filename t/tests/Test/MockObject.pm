...
sub deref {
  ...
  return @$scalar if ref $scalar eq "ARRAY";
  return $scalar;
}

# PATCH: Fix this so it actually lets you test the args
sub called_ok
{
  my($self, $method, $expected_args) = @_;
  my $call;
  for my $c (reverse @{ _calls($self) }) {
    if ($c->[0] eq $method) {
      $call = $c;
      last;
    }
  }
  my $actual_args = $call->[1];
  
  $Test->diag("Expected $method to be invoked, but wasn't") unless $call;
  unless ($Test->ok((deref($actual_args) == deref($expected_args)), $method)) {
    #$Test->diag("Expected $method to be invoked with " . Dumper($args) . ", but wasn't");
    my $msg = "Expected $method to be invoked with " . Dumper($expected_args) . ", but was called with " . Dumper($actual_args);
    $Test->diag($msg);
  }
}
